...
"""
Melt Stake PTV System Controller - systemd service startup script.

Automatic entry point meltstake_ptv as a
...
def build_command(venv_python: Path) -> list[str]:
    """Build the command list for running the meltstake_ptv module."""
    cmd = [
        str(venv_python),
        "-m", "meltstake_ptv",
        "--config", CONFIG_FILE,
